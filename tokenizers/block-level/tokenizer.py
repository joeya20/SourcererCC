...
def read_config():
    ...
    try:
        ...
        print(
            "ERROR - Config settings not found. Usage: $python this-script.py config-file.ini"
        )
        sys.exit()
...
def tokenize_files(
    ...
    file_string = "".join(
        [s for s in file_string.splitlines(True) if s.strip()]
    ).strip()
    ...
    tokens = ",".join(
        ["{}@@::@@{}".format(k, v) for k, v in file_string_for_tokenization.iteritems()]
    )
    ...
    if len(block_linenos) == 0:
        ...
        try:
            ...
            try:
                ...
                logging.info(
                    "Error on tokenize_blocks (1) (file,exception,input) (%s,%s,%s)"
                    % (file_path, e, file_string)
                )
            file_hash = m.hexdigest()
            hash_time = (dt.datetime.now() - h_time).microseconds
            lines = file_string.count("\n")
            ...
            # Remove tagged comments
            file_string = re.sub(
                comment_open_close_pattern, "", file_string, flags=re.DOTALL
            )
            # Remove end of line comments
            file_string = re.sub(
                comment_inline_pattern, "", file_string, flags=re.MULTILINE
            )
            ...
            for i, block_string in enumerate(blocks):
                ...
                try:
                    ...
                    logging.info(
                        "Error on tokenize_blocks (2) (file,exception,input) (%s,%s,%s)"
                        % (file_path, e, block_string)
                    )
                ...
                if not block_string.endswith("\n"):
                    block_lines += 1
                block_string = "".join(
                    [s for s in block_string.splitlines(True) if s.strip()]
                )
                ...
                # Remove tagged comments
                block_string = re.sub(
                    comment_open_close_pattern, "", block_string, flags=re.DOTALL
                )
                # Remove end of line comments
                block_string = re.sub(
                    comment_inline_pattern, "", block_string, flags=re.MULTILINE
                )
                re_time += (dt.datetime.now() - r_time).microseconds
                block_string = "".join(
                    [s for s in block_string.splitlines(True) if s.strip()]
                ...
                for sep in separators:
                    block_string_for_tokenization = (
                        block_string_for_tokenization.replace(sep, " ")
                    )
                se_time += (dt.datetime.now() - s_time).microseconds
                # Create a list of tokens
                ...
                # Count occurrences
                block_string_for_tokenization = collections.Counter(
                    block_string_for_tokenization
                )
                # Converting Counter to dict because according to StackOverflow is better
                ...
                tokens = ",".join(
                    [
                        "{}@@::@@{}".format(k, v)
                        for k, v in block_string_for_tokenization.items()
                    ]
                )
                token_time += (dt.datetime.now() - t_time).microseconds
                # MD5
                ...
                try:
                    ...
                    logging.info(
                        "Error on tokenize_blocks (3) (file,exception,input) (%s,%s,%s)"
                        % (file_path, e, tokens)
                    )
                hash_time += (dt.datetime.now() - h_time).microseconds
                block_tokens = (
            ...
            logging.warning(
                "Some error on tokenize_blocks for file %s. %s" % (file_path, e)
            )
...
def process_file_contents(
    ...
    logging.info(
        "Attempting to process_file_contents " + os.path.join(container_path, file_path)
    )
    ...
    if (project_format == "zipblocks") or (project_format == "folderblocks"):
        (final_stats, blocks_data, file_parsing_times) = tokenize_blocks(
            file_string,
            comment_inline_pattern,
            comment_open_close_pattern,
            separators,
        ...
        if (
            (final_stats is None)
            or (blocks_data is None)
            or (file_parsing_times is None)
        ):
            logging.warning(
                "Problems tokenizing file " + os.path.join(container_path, file_path)
            )
            return [0, 0, 0, 0, 0]
                ...
                FILE_tokens_file.write(
                    ",".join(
                        [
                            proj_id,
                            block_id,
                            str(tokens_count_total),
                            str(tokens_count_unique),
                            token_hash + tokens,
                        ]
                    )
                    + "\n"
        ...
        w_time += (dt.datetime.now() - ww_time).microseconds
    logging.info(
        "Successfully ran process_file_contents "
        + os.path.join(container_path, file_path)
    )
...
def process_regular_folder(
    ...
    base_file_id,
    FILE_tokens_file,
    FILE_bookkeeping_proj,
    FILE_stats_file,
    logging,
):
    zip_time = (
        file_time
    ) = string_time = tokens_time = hash_time = write_time = regex_time = 0

    logging.info("Attempting to process_regular_folder " + proj_path)

    result = [
        f
        for dp, dn, filenames in os.walk(proj_path)
        for f in filenames
    ...
    for file_path in result:
        ...
        try:
            my_file = io.open(
                os.path.join(proj_path, file_path), encoding="utf-8", errors="ignore"
            )
        ...
        if my_file is None:
            logging.warning(
                ...
                + os.path.join(proj_path, file_path)
                + "> (process "
                + str(process_num)
                + ")"
            )
        ...
        try:
            ...
            logging.warning(
                "Unable to read contents of file %s. %s "
                % (os.path.join(proj_path, file_path), e)
            )
            continue
        try:
            times = process_file_contents(
                file_string,
                proj_id,
                file_id,
                zip_file,
        ...
        with tarfile.open(tar_file, "r|*") as my_tar_file:
            for f in my_tar_file:
                ...
                try:
                    ...
                    logging.warning(
                        f"Unable to open file (1) <{proj_id},{str(file_id)},{os.path.join(tar_file, file_path)}> (process {str(process_num)}): {e}"
                    )
                    continue
                zip_time += (dt.datetime.now() - z_time).microseconds
        ...
        with zipfile.ZipFile(proj_path, "r") as my_file:
            for file in my_file.infolist():
                ...
                try:
                    ...
                    logging.warning(
                        f"Unable to open file (1) <{os.path.join(proj_path, file)}> (process {str(process_num)}): {e}"
                    )
                    continue
                zip_time += (dt.datetime.now() - z_time).microseconds
...
def process_one_project(
    ...
    logging.info(
        "Project finished <%s,%s> (process %s)", proj_id, proj_path, process_num
    )
    logging.info(
...
def process_projects(
    process_num, list_projects, base_file_id, global_queue, project_format
):
    ...
    file_handler = logging.FileHandler(
        os.path.join(PATH_logs, "LOG-" + str(process_num) + ".log")
    )
    ...
    global file_count
    file_count = 0
    with open(FILE_files_tokens_file, "a+") as FILE_tokens_file, open(
        FILE_bookkeeping_proj_name, "a+"
    ) as FILE_bookkeeping_proj, open(FILE_files_stats_file, "a+") as FILE_stats_file:
    ...
    logging.info(
        "Process %s finished. %s files in %ss.", process_num, file_count, p_elapsed
    )
...
def start_child(processes, global_queue, proj_paths, batch, project_format):
    ...
    p = Process(
        name="Process " + str(pid),
        target=process_projects,
        args=(
            pid,
            paths_batch,
            processes[pid][1],
...
def kill_child(processes, pid, n_files_processed):
    ...
    if processes[pid][0] is not None:
        ...
        print(
            "Process %s finished, %s files processed (%s). Current total: %s"
...
if __name__ == "__main__":
    ...
    if FILE_priority_projects is not None:
        ...
        prio_proj_paths = zip(
            range(init_proj_id, len(prio_proj_paths) + init_proj_id), prio_proj_paths
        )
    proj_paths = []

    if project_format in [
        "zipblocks"
    ]:  # zipblocks will diverge the process flow on process_file()
    ...
    if project_format in [
        "folderblocks"
    ]:  # folderblocks will diverge the process flow on process_file()
        print("'" + project_format + "'" + "format")
        with open(FILE_projects_list) as f:
            for line in f:
                proj_paths.append(line[:-1])
        proj_paths = list(zip(range(1, len(proj_paths) + 1), proj_paths))
    if (
        os.path.exists(PATH_stats_file_folder)
        or os.path.exists(PATH_bookkeeping_proj_folder)
        or os.path.exists(PATH_tokens_file_folder)
        or os.path.exists(PATH_logs)
    ):
        print(
            "ERROR - Folder ["
