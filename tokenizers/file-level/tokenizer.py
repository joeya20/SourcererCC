...
def tokenize_files(
    file_bytes, comment_inline_pattern, comment_open_close_pattern, separators
):
    final_stats = "ERROR"
    final_tokens = "ERROR"
    ...
    if obfuscate:
        ...
        tokens = ",".join(
            [
                "{}@@::@@{}".format(hashlib.md5(k.encode("utf-8")).hexdigest(), v)
                for k, v in file_tokens.items()
            ]
        )
    else:
        logging.info("Generating original tokens")
        tokens = ",".join(["{}@@::@@{}".format(k, v) for k, v in file_tokens.items()])
...
def process_file_contents(
    ...
    file_url = file_path.replace(
        " ", "%20"
    )  # proj_url + '/' + file_path[7:].replace(' ','%20')
...
if __name__ == "__main__":
    ...
    if FILE_priority_projects is not None:
        ...
        prio_proj_paths = list(
            zip(
                range(init_proj_id, len(prio_proj_paths) + init_proj_id),
                prio_proj_paths,
            )
        )
